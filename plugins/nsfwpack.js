...
const handler = async (m, { conn, command, usedPrefix }) => {
  ...
  switch (command) {
    case 'pack':
      ...
      conn.sendMessage(m.chat, { image: { url: url }, caption: `Command _Succeed_` }, { quoted: m });
      break;
    case 'pack2':
      const url2 = await packgirl[Math.floor(Math.random() * packgirl.length)];
      conn.sendMessage(m.chat, { image: { url: url2 }, caption: `Command _Succeed_` }, { quoted: m });
      break;
    case 'pack3':
      const url3 = await packmen[Math.floor(Math.random() * packmen.length)];
      conn.sendMessage(m.chat, { image: { url: url3 }, caption: `Command _Succeed_` }, { quoted: m });
      break;
    case 'videoxxx':
    case 'vídeoxxx':
      const url4 = await videosxxxc[Math.floor(Math.random() * videosxxxc.length)];
      await conn.sendMessage(m.chat, { video: { url: url4 }, caption: `Command _Succeed_` }, { quoted: m });
      break;
    case 'videoxxxlesbi':
    case 'videolesbixxx':
    case 'pornolesbivid':
    ...
    case 'pornolesv':
      ...
      await conn.sendMessage(m.chat, { video: { url: url5 }, caption: `Command _Succeed_` }, { quoted: m });
      break;
  }
};
