let handler = async (m, { conn, args }) => {
  ...
  const formatMoney = (value) => {
    ...
    let formatted = absValue.toLocaleString();
    if (absValue >= 1e9) formatted = (absValue / 1e9).toFixed(1).replace(/\.0$/, '') + 'M - ' + Math.round(absValue / 1e6).toLocaleString();
    else if (absValue >= 1e6) formatted = (absValue / 1e6).toFixed(1).replace(/\.0$/, '') + 'JT - ' + Math.round(absValue / 1e3).toLocaleString();
    else if (absValue >= 1e3) formatted = (absValue / 1e3).toFixed(1).replace(/\.0$/, '') + 'K - ' + absValue.toLocaleString();
