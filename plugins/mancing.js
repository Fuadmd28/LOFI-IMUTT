let handler = async (m, {
    ...
    let fishingrod = pengguna.fishingrod 
...
*- command* :  ${usedPrefix + command} easy
`.trim();
    ...
    try {
        if (/fishing|mancing/i.test(command)) {
            switch (type) {
                ...
                case 'emosi':
                    let __timers = (new Date - pengguna.lastmancingemosi)
                    ...
                    if (pengguna.lastmancingemosi > 10800000) throw m.reply(`Kamu masih kelelahan\nHarap tunggu *${timers}* lagi`)

                    if (pengguna.fishingrod > 0) {
                        if (new Date - pengguna.lastmancingemosi > 240000) {
...
${ usedPrefix }cook`
                ...
                case 'easy':
                    ...
                    if (fishingrod  == 0 || umpan == 0) return conn.reply(m.chat, `*[❗] kamu tidak punya kail pancingan dan umpan 100 🎣*`, m)
                    if (new Date - pengguna.lastmancingeasy > 28800000) {
                        if (pengguna.fishingrod  > 1) {
                    ...
                    return conn.reply(m.chat, list, m, {
                        contextInfo: {
                            externalAdReply: {
                                mediaType: 1,
                                title: 'AXELLDX',
                                thumbnailUrl: 'https://telegra.ph/file/12208f2bd02a4c832758b.jpg',
