let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    if(global.db.data.users[user].sahabat != m.sender){
      conn.reply(m.chat,`*Maaf @${user.split('@')[0]} tidak sedang mengajak berteman anda*`,m,{contextInfo: {
        mentionedJid: [user]
      }})
    }else{
      global.db.data.users[m.sender].sahabat = user
      conn.reply(m.chat,`*Selamat anda resmi bersahabat dengan @${user.split('@')[0]}*\n\n*Semoga rukun dan ceria selalu @${user.split('@')[0]} â™¡ @${m.sender.split('@')[0]} ðŸ¥³ðŸ¥³*`,m,{contextInfo: {
        mentionedJid: [m.sender,user]
      }})
    }
	}	
}
handler.help = ['terimasahabat *@tag*']
handler.tags = ['fun']
handler.command = /^(terimasahabat)$/i
handler.group = true
handler.limit = true
handler.fail = null
